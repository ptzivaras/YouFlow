...
export const createService = async (req: Request, res: Response) => {
  try {
    const { business_id, name, description, duration_minutes } = req.body;

    if (!business_id || !name || !duration_minutes) {
      res.status(400).json({ error: "business_id, name, and duration_minutes are required" });
      return;
    }
...
export const updateService = async (req: Request, res: Response) => {
  try {
    ...
    const service = await serviceService.updateService(id, { name, description, duration_minutes });
    res.status(200).json(service);
  } catch (error) {
    if (error instanceof Error && error.message === "Service not found") {
      res.status(404).json({ error: error.message });
